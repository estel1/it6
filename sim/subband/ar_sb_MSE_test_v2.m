...
F0_new_diag = get_sb_matrix_new_diag(N, w1, w2, 0 ) ;
F1_new_diag = get_sb_matrix_new_diag(N, w1, w2, 1 ) ;
F2_new_diag = get_sb_matrix_new_diag(N, w1, w2, 2 ) ;
fprintf('done\n') ;
...
parfor jj=1:length(SNR_dB)
    ...
    for k=1:num_of_tests
        ...
        r05 = sum(sum(sum(sum(xx4.*F0_new_diag)))) ;
        r15 = sum(sum(sum(sum(xx4.*F1_new_diag)))) ;
        r25 = sum(sum(sum(sum(xx4.*F2_new_diag)))) ;
        rxx5 = [r05, r15, r25 ] ;
...
%            ];
        ...
        %%%%%%%%%%%%
        % 5
        b5 = ar_model([rxx5(1); rxx5(2); rxx5(3)]) ;
        [poles5, omega0_5, Hjw0_5] = get_ar_pole(b5) ;
        freq5(jj) = freq5(jj) + (omega0_5*fd/2/pi - fs)^2 ;
    ...
    freq5(jj) = sqrt(freq5(jj) / num_of_tests)  ;
...
SNR_dB_acf = SNR_dB ;
save('freq_sko_ar', 'freq1', 'freq2', 'freq3', 'freq4', 'freq5', 'SNR_dB_acf')
...
semilogy(SNR_dB, freq5, '-m^', 'LineWidth', 3, 'MarkerSize', 8, 'Color', [0.7 0.1 0.1]  ) ;
set(gca,'FontSize',14) ;
title(sprintf('Band:-0.5\\pi...0.5\\pi, D=3, N=%d',N)) ,
legend('1', '2', '3', 'SB', 'SB\_new\_diag') ;
grid on ;
xlabel('SNR, dB') ;
ylabel('\sigma^2') ;
